...
public class EntityUtils {
    private static Cache<String, WeakReference<Entity>> cachedEntities = Caffeine.newBuilder().expireAfterWrite(Duration.ofSeconds(1)).build();
...
//                        if (! chunk.isEntitiesLoaded()) continue;
    ...
    public static void collectEntitiesThenDoSet(Consumer<Collection<Entity>> consumer) {
        TaskManager.runTask(() -> {
            consumer.accept(getEntities(true).values().stream().map(WeakReference::get).filter(Objects::nonNull).collect(Collectors.toList()));
        });
    }
    ...
    public static Cache<String, WeakReference<Entity>> getCachedEntities() {
        return EntityUtils.cachedEntities;
    }

    public static void setCachedEntities(final Cache<String, WeakReference<Entity>> cachedEntities) {
        EntityUtils.cachedEntities = cachedEntities;
    }

    public static EntityLookupTimer getLookupTimer() {
        return EntityUtils.lookupTimer;
    }

    public static void setLookupTimer(final EntityLookupTimer lookupTimer) {
